...
final class Netty4SizeHeaderFrameDecoder extends ByteToMessageDecoder {
    ...
    protected void decode(ChannelHandlerContext ctx, ByteBuf in, List<Object> out) throws Exception {
        try {
            logger.trace("N4SHFD.decode(): decoding {} bytes", in.readableBytes());
            ...
            if (!continueProcessing) {
                logger.trace("N4SHFD.decode(): !continueProcessing");
                return;
            }
            out.add(message);
        } catch (IllegalArgumentException ex) {
            logger.trace("N4SHFD.decode(): IllegalArgumentException");
            throw new TooLongFrameException(ex);
        } catch (IllegalStateException ex) {
            logger.trace("N4SHFD.decode(): IllegalStateException");
