        ...
        if (endOfResponse) {
            ...
            ActionListener.run(
                ActionListener.assertOnce(
                    new ContextPreservingActionListener<>(threadContext.newRestorableContext(false), new ActionListener<>() {
                        @Override
                        ...
                        private void checkShutdown() {
                            if (channel.eventLoop().isShuttingDown()) {
                                // The event loop is shutting down, and https://github.com/netty/netty/issues/8007 means that we cannot
                                // know if the preceding activity made it onto its queue before shutdown or whether it will just vanish
                                // without a trace, so to avoid a leak we must double-check that the final listener is completed once
                                // the event loop is terminated. Note that the final listener came from Netty4Utils#safeWriteAndFlush so
                                // its executor is an ImmediateEventExecutor which means this completion is not subject to the same
                                // issue, it still works even if the event loop has already terminated.
                                channel.eventLoop()
                                    .terminationFuture()
