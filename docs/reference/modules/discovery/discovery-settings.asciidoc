...
(<<static-cluster-setting,Static>>)
When a node requests to join the cluster, the elected master node sends it a copy
of a recent cluster state to detect certain problems which might prevent it from
joining the cluster. The master caches the state it sends to reduce the work needed
when multiple nodes all join the cluster in quick succession. This setting controls
how long the master waits until it clears its cache. Defaults to `60s`.
