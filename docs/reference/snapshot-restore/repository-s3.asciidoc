...
`metadata_storage_class`::
    ...
    Values may be `standard`, `reduced_redundancy`, `standard_ia`, `onezone_ia`
    and `intelligent_tiering`. If unset, metadata objects use the same storage
...
`storage_class` repository setting, and may also specify a different storage
class for metadata objects with the `metadata_storage_class` repository
setting. The `standard` storage class is well-suited to metadata objects since
they are small and accessed frequently. In contrast, the larger data objects
are accessed less frequently so a different storage class may be more suitable.
