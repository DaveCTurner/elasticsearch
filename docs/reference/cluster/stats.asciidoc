...
`snapshot_activity`::
(object)
Contains statistics about the current <<snapshot-restore,snapshot>> activity in the cluster.
+
.Properties of `snapshot_activity`
[%collapsible%open]
=====

`in_progress_counts`:::
(object)
Contains statistics which report the number of various ongoing snapshot activities in the cluster.
+
.Properties of `in_progress_counts`
[%collapsible%open]
======
`snapshots`:::
(integer)
...
`totals`:::
(object)
Contains statistics which report the overall progress of snapshot activities for a repository.
+
.Properties of `totals`
[%collapsible%open]
=======
`snapshots`:::
(integer)
...
// TESTRESPONSE[s/"snapshot_activity": \{[^\}]*\}/"snapshot_activity": $body.$_path/]
...
//    `packaging_types` and `snapshot_activity`.
