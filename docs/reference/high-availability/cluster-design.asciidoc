...
=== Examples of resilient architectures
...
voting-only master-eligible node>>. A dedicated tiebreaker need not be as
powerful as the other two nodes since it has no other roles and will not
perform any searches nor coordinate any client requests nor be elected as the
...
supply or network router. If so, you should also plan for the failure of this
...
==== Two-zone deployments with a tiebreaker
...
master-eligible node>>. A dedicated tiebreaker need not be as powerful as the
other two nodes since it has no other roles and will not perform any searches
nor coordinate any client requests nor be elected as the master of the cluster.
...
==== Three-or-more-zone deployments
