...
public abstract class PeerFinder extends AbstractLifecycleComponent {
    ...
    private final ObjLongConsumer<Runnable> futureExecutor;
    ...
    PeerFinder(Settings settings, TransportService transportService, UnicastHostsProvider hostsProvider,
               ObjLongConsumer<Runnable> futureExecutor, TransportAddressConnector transportAddressConnector,
    ...
    public void activate() {
        ...
        assert Thread.holdsLock(mutex) == false : "Peerfinder mutex is held in error";
    ...
    private ActivePeerFinder getActivePeerFinder() {
        assert Thread.holdsLock(mutex) : "Peerfinder mutex not held";
    ...
    public boolean foundQuorumFrom(VotingConfiguration votingConfiguration) {
        synchronized (mutex) {
            return votingConfiguration.hasQuorum(
                getActivePeerFinder().foundPeers.keySet().stream().map(DiscoveryNode::getId).collect(Collectors.toSet()));
        }
    }
    ...
    private class ActivePeerFinder {
        ...
        private void handleWakeUpUnderLock(DiscoveryNodes lastAcceptedNodes) {
            ...
            futureExecutor.accept(new Runnable() {
                @Override
                public void run() {
                    handleWakeUp();
                }
        ...
        void startProbe(TransportAddress transportAddress) {
            ...
            if (cachedNode != null) {
                ...
                // TODO this should be tested, but currently LegislatorTests never hit this
