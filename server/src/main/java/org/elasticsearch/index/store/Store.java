...
public class Store extends AbstractIndexShardComponent implements Closeable, RefCounted {
    ...
    public Optional<SequenceNumbers.CommitInfo> findSafeIndexCommit(long globalCheckpoint) throws IOException {
        ...
        // SDH E-10233: this checks the *safe* commit from CombinedDeletionPolicy.findSafeCommitPoint
        // (highest commit whose maxSeqNo <= GCP), not necessarily the last commit. A later commit
        // can already have max_seq_no >> GCP and LCP stuck at a hole. If an older commit still
        // satisfies maxSeqNo <= GCP, this returns that older commit and recoverLocallyUpToGlobalCheckpoint
        // proceeds ops-based (0 files). openEngineAndSkipTranslogRecovery then opens the last commit,
        // so the hole in the last commit is kept and phase2 only sends ops from GCP+1.
