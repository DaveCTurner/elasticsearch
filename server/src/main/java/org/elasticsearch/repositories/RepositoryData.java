...
public final class RepositoryData {
    ...
    public XContentBuilder snapshotsToXContent(
        ...
        if (shouldWriteShardGens) {
            ...
            final Version repoVersion;
            if (shouldWriteRepoUuid) {
                repoVersion = SnapshotsService.REPOSITORY_UUID_IN_REPO_DATA_VERSION;
            } else if (shouldWriteIndexGens) {
                repoVersion = SnapshotsService.INDEX_GEN_IN_REPO_DATA_VERSION;
            } else {
                repoVersion = SnapshotsService.SHARD_GEN_IN_REPO_DATA_VERSION;
            }
            builder.field(MIN_VERSION, repoVersion.toString());
        }
