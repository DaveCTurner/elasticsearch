...
public interface Repository extends LifecycleComponent {
    ...
    /**
     * Called during verification after all the nodes have completed the {@link #verify} step, so the master can check that all the nodes
     * wrote the blobs we expected them to write.
     *
     * @param verificationToken value returned by {@link Repository#startVerification()}
     * @param nodes             the nodes that have completed the {@link #verify} step.
     * @return {@code nodes}
     */
    List<DiscoveryNode> verifyNodes(String verificationToken, List<DiscoveryNode> nodes);
