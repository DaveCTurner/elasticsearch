...
public class SearchPreferenceIT extends ESIntegTestCase {
    ...
    public void testStopOneNodePreferenceWithRedState() throws IOException {
        ...
        clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForStatus(ClusterHealthStatus.RED).get();
    ...
    public void testSimplePreference() {
        indicesAdmin().prepareCreate(masterNodeTimeout, "test").setSettings("{\"number_of_replicas\": 1}", XContentType.JSON).get();
        ensureGreen();
