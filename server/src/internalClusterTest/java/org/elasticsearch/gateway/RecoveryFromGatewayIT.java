...
public class RecoveryFromGatewayIT extends ESIntegTestCase {
    ...
    public void testLatestVersionLoaded() throws Exception {
        ...
        String metadataUuid = clusterAdmin().prepareState(masterNodeTimeout).execute().get().getState().getMetadata().clusterUUID();
        ...
        indicesAdmin().preparePutTemplate(masterNodeTimeout, "template_1")
            ...
            .get();
        indicesAdmin().prepareAliases(masterNodeTimeout).addAlias("test", "test_alias", QueryBuilders.termQuery("field", "value")).get();
        ...
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).execute().get().getState().getMetadata().clusterUUID(),
