...
public class UnsafeBootstrapAndDetachCommandIT extends ESIntegTestCase {
    ...
    public void testNoInitialBootstrapAfterDetach() throws Exception {
        ...
        ClusterState state = internalCluster().client().admin().cluster().prepareState(masterNodeTimeout).setLocal(true).get().getState();
        assertTrue(state.blocks().hasGlobalBlockWithId(NoMasterBlockService.NO_MASTER_BLOCK_ID));
    ...
    public void testCanRunUnsafeBootstrapAfterErroneousDetachWithoutLoosingMetadata() throws Exception {
        ...
        ClusterState state = internalCluster().client().admin().cluster().prepareState(masterNodeTimeout).get().getState();
        ...
        state = internalCluster().client().admin().cluster().prepareState(masterNodeTimeout).get().getState();
