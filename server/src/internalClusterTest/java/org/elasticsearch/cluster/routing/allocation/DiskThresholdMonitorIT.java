...
public class DiskThresholdMonitorIT extends DiskUsageIntegTestCase {
    ...
    public void testFloodStageExceeded() throws Exception {
        ...
        assertBusy(() -> {
            final ShardRouting primaryShard = clusterAdmin().prepareState(masterNodeTimeout)
                .clear()
        ...
        refreshClusterInfo();
        assertFalse(clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForEvents(Priority.LANGUID).get().isTimedOut());
    ...
    private static String getIndexBlock(String indexName, String blockName) {
        return indicesAdmin().prepareGetSettings(masterNodeTimeout, indexName).setNames(blockName).get().getSetting(indexName, blockName);
    }
