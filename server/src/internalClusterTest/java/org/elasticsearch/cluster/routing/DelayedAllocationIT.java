...
public class DelayedAllocationIT extends ESIntegTestCase {
    ...
    public void testNoDelayedTimeout() throws Exception {
        ...
        assertThat(clusterAdmin().prepareHealth(masterNodeTimeout).get().getDelayedUnassignedShards(), equalTo(0));
        ensureGreen("test");
    }
    ...
    public void testDelayedAllocationNodeLeavesAndComesBack() throws Exception {
        ...
        assertBusy(
            () -> assertThat(
                clusterAdmin().prepareState(masterNodeTimeout).all().get().getState().getRoutingNodes().unassigned().size() > 0,
                equalTo(true)
            )
        );
