...
public class ClusterSettingsIT extends ESIntegTestCase {
    ...
    private void testClusterNonExistingSettingsUpdate(
        ...
        int value1 = 10;
        try {
            ClusterUpdateSettingsRequestBuilder builder = clusterAdmin().prepareUpdateSettings(masterNodeTimeout);
    ...
    private void testDeleteIsAppliedFirst(
        ...
        ClusterUpdateSettingsRequestBuilder undoBuilder = clusterAdmin().prepareUpdateSettings(masterNodeTimeout);
        consumer.accept(
    ...
    public void testResetClusterTransientSetting() {
        ...
        ClusterUpdateSettingsResponse response = clusterAdmin().prepareUpdateSettings(masterNodeTimeout)
        ...
        response = clusterAdmin().prepareUpdateSettings(masterNodeTimeout)
    ...
    public void testClusterSettingsUpdateResponse() {
        ...
        ClusterUpdateSettingsResponse response1 = clusterAdmin().prepareUpdateSettings(masterNodeTimeout)
        ...
        ClusterUpdateSettingsResponse response2 = clusterAdmin().prepareUpdateSettings(masterNodeTimeout)
        ...
        ClusterUpdateSettingsResponse response3 = clusterAdmin().prepareUpdateSettings(masterNodeTimeout)
    ...
    private void testUpdateSettings(
        ...
        ClusterUpdateSettingsRequestBuilder initialBuilder = clusterAdmin().prepareUpdateSettings(masterNodeTimeout);
        ...
        try {
            ClusterUpdateSettingsRequestBuilder badBuilder = clusterAdmin().prepareUpdateSettings(masterNodeTimeout);
    ...
    private void testRemoveArchiveSettingsWithBlocks(boolean readOnly, boolean readOnlyAllowDelete) throws Exception {
        ...
        state = clusterAdmin().prepareState(masterNodeTimeout).get().getState();
        ...
        // cannot remove read only block due to archived settings
        {
            ...
            final IllegalArgumentException e1 = expectThrows(
                ...
                clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setPersistentSettings(builder).setTransientSettings(builder)
            );
        ...
        final ClusterBlockException e3 = expectThrows(
            ClusterBlockException.class,
            clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setPersistentSettings(Settings.builder().putNull("archived.*"))
        );
        if (readOnly) {
...

        {
            ...
            final ClusterBlockException e4 = expectThrows(
                ClusterBlockException.class,
                clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setPersistentSettings(builder)
            );
            if (readOnly) {
        ...
        clearOrSetFalse(builder, readOnlyAllowDelete, Metadata.SETTING_READ_ONLY_ALLOW_DELETE_SETTING);
        assertAcked(
            clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setPersistentSettings(builder).setTransientSettings(builder).get()
        );
    ...
    public void testClusterUpdateSettingsWithBlocks() {
        ...
        ClusterUpdateSettingsRequestBuilder request = clusterAdmin().prepareUpdateSettings(masterNodeTimeout)
        ...
        try {
            ...
            assertAcked(clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setTransientSettings(settings).get());
        ...
        try {
            ...
            assertAcked(clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setTransientSettings(s).get());
        }
    ...
    private void testLoggerLevelUpdate(final BiConsumer<Settings.Builder, ClusterUpdateSettingsRequestBuilder> consumer) {
        ...
        ClusterUpdateSettingsRequestBuilder throwBuilder = clusterAdmin().prepareUpdateSettings(masterNodeTimeout);
        ...
        try {
            ...
            ClusterUpdateSettingsRequestBuilder updateBuilder = clusterAdmin().prepareUpdateSettings(masterNodeTimeout);
    ...
    public void testUserMetadata() {
        ...
        builder.get();
        ClusterStateResponse state = clusterAdmin().prepareState(masterNodeTimeout).get();
        ...
        ClusterStateResponse updatedState = clusterAdmin().prepareState(masterNodeTimeout).get();
