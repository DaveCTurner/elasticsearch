...
public class SimpleClusterStateIT extends ESIntegTestCase {
    ...
    public void testRoutingTable() throws Exception {
        ClusterStateResponse clusterStateResponseUnfiltered = clusterAdmin().prepareState(masterNodeTimeout)
            .clear()
        ...
        ClusterStateResponse clusterStateResponse = clusterAdmin().prepareState(masterNodeTimeout).clear().get();
    ...
    public void testNodes() throws Exception {
        ClusterStateResponse clusterStateResponse = clusterAdmin().prepareState(masterNodeTimeout).clear().setNodes(true).get();
        assertThat(clusterStateResponse.getState().nodes().getNodes().size(), is(cluster().size()));

        ClusterStateResponse clusterStateResponseFiltered = clusterAdmin().prepareState(masterNodeTimeout).clear().get();
    ...
    public void testMetadataVersion() {
        createIndex("index-1");
        createIndex("index-2");
        long baselineVersion = clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().version();
        assertThat(baselineVersion, greaterThan(0L));
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).setIndices("index-1").get().getState().metadata().version(),
            greaterThanOrEqualTo(baselineVersion)
        );
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).setIndices("index-2").get().getState().metadata().version(),
            greaterThanOrEqualTo(baselineVersion)
        );
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).setIndices("*").get().getState().metadata().version(),
            greaterThanOrEqualTo(baselineVersion)
        );
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).setIndices("not-found").get().getState().metadata().version(),
            greaterThanOrEqualTo(baselineVersion)
        );
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).clear().setMetadata(false).get().getState().metadata().version(),
            equalTo(0L)
        );
    }
    ...
    public void testIndexTemplates() throws Exception {
        ...
        ClusterStateResponse clusterStateResponseUnfiltered = clusterAdmin().prepareState(masterNodeTimeout).get();
    ...
    private void testFilteringByIndexWorks(String[] indices, String[] expected) {
        ClusterStateResponse clusterState = clusterAdmin().prepareState(masterNodeTimeout)
            .clear()
            .setMetadata(true)
            .setRoutingTable(true)
            .setIndices(indices)
    ...
    public void testLargeClusterStatePublishing() throws Exception {
        ...
        MappingMetadata masterMappingMetadata = indicesAdmin().prepareGetMappings(masterNodeTimeout, "test")
            .get()
            .getMappings()
            .get("test");
        for (Client client : clients()) {
            MappingMetadata mappingMetadata = client.admin()
                .indices()
                .prepareGetMappings(masterNodeTimeout, "test")
                .setLocal(true)
                .get()
                .getMappings()
                .get("test");
    ...
    public void testIndicesOptions() throws Exception {
        ...
        clusterStateResponse = clusterAdmin().prepareState(masterNodeTimeout).clear().setMetadata(true).setIndices("f*").get();
        ...
        clusterStateResponse = clusterAdmin().prepareState(masterNodeTimeout)
            .clear()
            .setMetadata(true)
            .setIndices("f*")
    ...
    public void testPrivateCustomsAreExcluded() throws Exception {
        ...
        ClusterStateResponse clusterStateResponse = clusterAdmin().prepareState(masterNodeTimeout).setCustoms(true).get();
