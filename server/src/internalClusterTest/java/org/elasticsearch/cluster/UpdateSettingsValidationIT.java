...
public class UpdateSettingsValidationIT extends ESIntegTestCase {
    public void testUpdateSettingsValidation() throws Exception {
        ...
        ClusterHealthResponse healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout, "test")
        ...
        healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout, "test")
            .setWaitForEvents(Priority.LANGUID)
            .setWaitForGreenStatus()
            .get();
        assertThat(healthResponse.isTimedOut(), equalTo(false));
