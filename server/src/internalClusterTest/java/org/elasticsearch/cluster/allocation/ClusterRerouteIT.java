...
public class ClusterRerouteIT extends ESIntegTestCase {
    ...
    private void rerouteWithCommands(Settings commonSettings) throws Exception {
        ...
        if (randomBoolean()) {
            indicesAdmin().prepareClose(masterNodeTimeout, "test").setWaitForActiveShards(ActiveShardCount.NONE).get();
        }
        ...
        state = clusterAdmin().prepareReroute(masterNodeTimeout)
        ...
        ClusterHealthResponse healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout)
            .setIndices("test")
    ...
    public void testDelayWithALargeAmountOfShards() throws Exception {
        ...
        ClusterHealthResponse healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForNodes("4").get();
        assertThat(healthResponse.isTimedOut(), equalTo(false));
    ...
    private void rerouteWithAllocateLocalGateway(Settings commonSettings) throws Exception {
        ...
        ClusterHealthResponse healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForNodes("2").get();
        assertThat(healthResponse.isTimedOut(), equalTo(false));
        ...
        assertThat(
            clusterAdmin().prepareHealth(masterNodeTimeout).setIndices("test").setWaitForNodes("2").get().getStatus(),
    ...
    public void testRerouteExplain() {
        ...
        ClusterHealthResponse healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForNodes("1").get();
        assertThat(healthResponse.isTimedOut(), equalTo(false));
        ...
        assertThat(cluster().size(), equalTo(2));
        healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForNodes("2").get();
        assertThat(healthResponse.isTimedOut(), equalTo(false));
        ...
        ClusterRerouteResponse resp = clusterAdmin().prepareReroute(masterNodeTimeout).add(cmd).setExplain(true).get();
    ...
    public void testMessageLogging() {
        ...
        try (var ignored = dryRunMockLog.capturing(TransportClusterRerouteAction.class)) {
            ...
            ClusterRerouteResponse dryRunResponse = clusterAdmin().prepareReroute(masterNodeTimeout)
                .setExplain(randomBoolean())
                .setDryRun(true)
        ...
        try (var ignored = allocateMockLog.capturing(TransportClusterRerouteAction.class)) {
            ...
            ClusterRerouteResponse response = clusterAdmin().prepareReroute(masterNodeTimeout)
    ...
    public void testClusterRerouteWithBlocks() {
        ...
        if (randomBoolean()) {
            assertAcked(indicesAdmin().prepareClose(masterNodeTimeout, "test-blocks"));
        }
