...
public class FilteringAllocationIT extends ESIntegTestCase {
    ...
    public void testAutoExpandReplicasToFilteredNodes() {
        ...
        logger.info("--> verify all are allocated on node1 now");
        clusterState = clusterAdmin().prepareState(masterNodeTimeout).get().getState();
    ...
    public void testTransientSettingsStillApplied() {
        ...
        clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setTransientSettings(exclude).get();
        ...
        clusterAdmin().prepareUpdateSettings(masterNodeTimeout).setPersistentSettings(other).setTransientSettings(exclude).get();
