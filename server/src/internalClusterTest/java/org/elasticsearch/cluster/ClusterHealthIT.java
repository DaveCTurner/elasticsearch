...
public class ClusterHealthIT extends ESIntegTestCase {
    ...
    public void testHealth() {
        ...
        ClusterHealthResponse healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout, "test1")
        ...
        healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout)
        ...
        healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout, "test1")
            .setWaitForGreenStatus()
            .setTimeout(TimeValue.timeValueSeconds(10))
            .get();
        assertThat(healthResponse.isTimedOut(), equalTo(false));
        ...
        healthResponse = clusterAdmin().prepareHealth(masterNodeTimeout, "test1", "test2")
            .setWaitForYellowStatus()
            .setTimeout(TimeValue.timeValueSeconds(1))
            .get();
        assertThat(healthResponse.isTimedOut(), equalTo(true));
    ...
    public void testHealthWithClosedIndices() {
        createIndex("index-1");
        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout).setWaitForGreenStatus().get();
        ...
        createIndex("index-2");
        assertAcked(indicesAdmin().prepareClose(masterNodeTimeout, "index-2"));
        ...
        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout, "index-1").get();
            assertThat(response.getStatus(), equalTo(ClusterHealthStatus.GREEN));
            assertThat(response.isTimedOut(), equalTo(false));
        ...
        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout, "index-2").get();
            assertThat(response.getStatus(), equalTo(ClusterHealthStatus.GREEN));
            assertThat(response.isTimedOut(), equalTo(false));
            assertThat(response.getIndices().size(), equalTo(1));
            assertThat(response.getIndices().get("index-2").getStatus(), equalTo(ClusterHealthStatus.GREEN));
        ...
        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout, "index-*").get();
            assertThat(response.getStatus(), equalTo(ClusterHealthStatus.GREEN));
            assertThat(response.isTimedOut(), equalTo(false));
            assertThat(response.getIndices().size(), equalTo(2));
            assertThat(response.getIndices().get("index-1").getStatus(), equalTo(ClusterHealthStatus.GREEN));
        ...
        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout, "index-*")
        ...
        assertAcked(indicesAdmin().prepareClose(masterNodeTimeout, "index-3"));

        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout)
        ...
        {
            ClusterHealthResponse response = clusterAdmin().prepareHealth(masterNodeTimeout, "index-3").get();
            assertThat(response.getStatus(), equalTo(ClusterHealthStatus.YELLOW));
            assertThat(response.isTimedOut(), equalTo(false));
            assertThat(response.getIndices().size(), equalTo(1));
            assertThat(response.getIndices().get("index-3").getStatus(), equalTo(ClusterHealthStatus.YELLOW));
        ...
        Thread clusterHealthThread = new Thread() {
            ...
            public void run() {
                while (finished.get() == false) {
                    ClusterHealthResponse health = clusterAdmin().prepareHealth(masterNodeTimeout).get();
    ...
    public void testWaitForEventsRetriesIfOtherConditionsNotMet() {
        final ActionFuture<ClusterHealthResponse> healthResponseFuture = clusterAdmin().prepareHealth(masterNodeTimeout, "index")
            .setWaitForEvents(Priority.LANGUID)
            .setWaitForGreenStatus()
            .execute();
        ...
        try {
            createIndex("index");
            assertFalse(clusterAdmin().prepareHealth(masterNodeTimeout, "index").setWaitForGreenStatus().get().isTimedOut());
    ...
    public void testWaitForEventsTimesOutIfMasterBusy() {
        ...
        try {
            final ClusterHealthResponse clusterHealthResponse = clusterAdmin().prepareHealth(masterNodeTimeout)
                .setWaitForEvents(Priority.LANGUID)
                .setTimeout(TimeValue.timeValueSeconds(1))
