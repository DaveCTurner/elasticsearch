...
public class RepositoriesIT extends AbstractSnapshotIntegTestCase {
    public void testRepositoryCreation() throws Exception {
        ...
        clusterStateResponse = client.admin().cluster().prepareState(masterNodeTimeout).clear().setMetadata(true).get();
        ...
        assertThat(
            client.admin()
                .cluster()
                .preparePutRepository(masterNodeTimeout, "test-repo-1")
                .setType("fs")
        ...
        assertEquals(beforeStateUuid, client.admin().cluster().prepareState(masterNodeTimeout).clear().get().getState().stateUUID());
    ...
    public void testMisconfiguredRepository() {
        ...
        try {
            client.admin().cluster().preparePutRepository(masterNodeTimeout, "test-repo").setType("fs").get();
    ...
    public void testRepositoryAckTimeout() {
        ...
        AcknowledgedResponse putRepositoryResponse = clusterAdmin().preparePutRepository(masterNodeTimeout, "test-repo-1")
            .setType("fs")
            .setSettings(
                Settings.builder()
        ...
        putRepositoryResponse = clusterAdmin().preparePutRepository(masterNodeTimeout, "test-repo-2")
            .setType("fs")
            .setSettings(
                Settings.builder()
                    .put("location", randomRepoPath())
        ...
        AcknowledgedResponse deleteRepositoryResponse = clusterAdmin().prepareDeleteRepository(masterNodeTimeout, "test-repo-2")
            .setTimeout(TimeValue.ZERO)
            .get();
        ...
        deleteRepositoryResponse = clusterAdmin().prepareDeleteRepository(masterNodeTimeout, "test-repo-1").get();
    ...
    public void testRepositoryConflict() throws Exception {
        ...
        clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, repo, snapshot1).setWaitForCompletion(true).get();
        String blockedNode = internalCluster().getMasterName();
        blockMasterOnWriteIndexFile(repo);
        logger.info("--> start deletion of snapshot");
        ActionFuture<AcknowledgedResponse> future = clusterAdmin().prepareDeleteSnapshot(masterNodeTimeout, repo, snapshot1).execute();
        ...
        RepositoryConflictException e1 = expectThrows(
            ...
            clusterAdmin().prepareDeleteRepository(masterNodeTimeout, repo)
        );
    ...
    public void testLeakedStaleIndicesAreDeletedBySubsequentDelete() throws Exception {
        ...
        client.admin().cluster().prepareDeleteSnapshot(masterNodeTimeout, repositoryName, snapshot2Name).get();
        ...
        client.admin().cluster().prepareDeleteSnapshot(masterNodeTimeout, repositoryName, snapshot1Name).get();
    ...
    public void testCleanupStaleBlobsConcurrency() throws Exception {
        ...
        client.admin().cluster().prepareDeleteSnapshot(masterNodeTimeout, repositoryName, snapshotName).execute(deleteFuture);
