...
public class RestoreSnapshotIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testParallelRestoreOperations() {
        ...
        RestoreSnapshotResponse restoreSnapshotResponse1 = client.admin()
            .cluster()
            .prepareRestoreSnapshot(masterNodeTimeout, repoName, snapshotName1)
        ...
        RestoreSnapshotResponse restoreSnapshotResponse2 = client.admin()
            .cluster()
            .prepareRestoreSnapshot(masterNodeTimeout, repoName, snapshotName2)
            .setWaitForCompletion(false)
    ...
    public void testRestoreIncreasesPrimaryTerms() {
        ...
        final IndexMetadata indexMetadata = clusterAdmin().prepareState(masterNodeTimeout)
            .clear()
        ...
        final IndexMetadata restoredIndexMetadata = clusterAdmin().prepareState(masterNodeTimeout)
            .clear()
            .setIndices(indexName)
            .setMetadata(true)
            .get()
    ...
    public void testRestoreWithDifferentMappingsAndSettings() throws Exception {
        ...
        MappingMetadata mappings = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .getMetadata()
            .getIndices()
        ...
        GetSettingsResponse getSettingsResponse = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test-idx").get();
    ...
    public void testRestoreAliases() throws Exception {
        ...
        assertAcked(indicesAdmin().prepareAliases(masterNodeTimeout).addAlias("test-idx-3", "alias-3"));
    ...
    public void testRestoreTemplates() throws Exception {
        ...
        assertThat(indicesAdmin().prepareDeleteTemplate(masterNodeTimeout, "test-template").get().isAcknowledged(), equalTo(true));
        GetIndexTemplatesResponse getIndexTemplatesResponse = indicesAdmin().prepareGetTemplates(masterNodeTimeout).get();
    ...
    public void testChangeSettingsOnRestore() throws Exception {
        ...
        GetSettingsResponse getSettingsResponse = client.admin().indices().prepareGetSettings(masterNodeTimeout, "test-idx").get();
        ...
        getSettingsResponse = client.admin().indices().prepareGetSettings(masterNodeTimeout, "test-idx").get();
        assertThat(getSettingsResponse.getSetting("test-idx", INDEX_REFRESH_INTERVAL_SETTING.getKey()), equalTo("5s"));
        // Make sure that number of shards didn't change
        assertThat(getSettingsResponse.getSetting("test-idx", SETTING_NUMBER_OF_SHARDS), equalTo("" + numberOfShards));
    ...
    public void testForbidDisableSoftDeletesDuringRestore() throws Exception {
        ...
        final SnapshotRestoreException restoreError = expectThrows(
            ...
            clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, "test-repo", "snapshot-0")
    ...
    public void testFailOnAncientVersion() throws Exception {
        ...
        final SnapshotRestoreException snapshotRestoreException = expectThrows(
            SnapshotRestoreException.class,
            clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repoName, oldSnapshot)
        );
        assertThat(
