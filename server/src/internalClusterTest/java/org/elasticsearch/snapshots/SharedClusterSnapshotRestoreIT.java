...
public class SharedClusterSnapshotRestoreIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testBasicWorkFlow() throws Exception {
        ...
        if (snapshotClosed) {
            assertAcked(
                indicesAdmin().prepareClose(masterNodeTimeout, indicesToSnapshot).setWaitForActiveShards(ActiveShardCount.ALL).get()
            );
        }

        createSnapshot("test-repo", "test-snap", Arrays.asList(indicesToSnapshot));

        List<SnapshotInfo> snapshotInfos = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
        ...
        if (snapshotClosed) {
            assertAcked(
                indicesAdmin().prepareOpen(masterNodeTimeout, indicesToSnapshot).setWaitForActiveShards(ActiveShardCount.ALL).get()
            );
        }
        ...
        indicesAdmin().prepareClose(masterNodeTimeout, "test-idx-1", "test-idx-2").get();
        ...
        assertNull(
            indicesAdmin().prepareGetSettings(masterNodeTimeout, "test-idx-1")
                .get()
        ...
        restoreSnapshotResponse = clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, "test-repo", "test-snap")
            .setWaitForCompletion(true)
    ...
    public void testSingleGetAfterRestore() throws Exception {
        ...
        RestoreSnapshotResponse restoreSnapshotResponse = clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repoName, snapshotName)
            .setWaitForCompletion(true)
    ...
    public void testFreshIndexUUID() {
        ...
        createIndex("test");
        String originalIndexUUID = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test")
            .get()
        ...
        ensureGreen();
        String newIndexUUID = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test")
            .get()
            .getSetting("test", IndexMetadata.SETTING_INDEX_UUID);
        ...
        ensureGreen();
        String newAfterRestoreIndexUUID = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test")
            .get()
            .getSetting("test", IndexMetadata.SETTING_INDEX_UUID);
        assertTrue(
        ...
        String copyRestoreUUID = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test-copy")
            .get()
    ...
    public void testDataFileFailureDuringSnapshot() throws Exception {
        ...
        logger.info("--> snapshot");
        CreateSnapshotResponse createSnapshotResponse = clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, "test-repo", "test-snap")
            .setWaitForCompletion(true)
            .setIndices("test-idx")
            .get();
        if (createSnapshotResponse.getSnapshotInfo().totalShards() == createSnapshotResponse.getSnapshotInfo().successfulShards()) {
    ...
    public void testDataFileFailureDuringRestore() throws Exception {
        ...
        if (restoreInfo.successfulShards() == restoreInfo.totalShards()) {
            ...
            final ClusterStateResponse clusterStateResponse = client.admin().cluster().prepareState(masterNodeTimeout).get();
    ...
    public void testUnrestorableIndexDuringRestore() throws Exception {
        ...
        Runnable fixupAction = () -> {
            ...
            assertAcked(clusterAdmin().prepareReroute(masterNodeTimeout).setRetryFailed(true));
        };
    ...
    private void unrestorableUseCase(
        ...
        RestoreSnapshotResponse restoreResponse = clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, "test-repo", "test-snap")
        ...
        ClusterStateResponse clusterStateResponse = clusterAdmin().prepareState(masterNodeTimeout)
            .setCustoms(true)
        ...
        restoreResponse = clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, "test-repo", "test-snap")
            .setWaitForCompletion(true)
            .get();
        assertThat(restoreResponse.getRestoreInfo().totalShards(), equalTo(numShards.numPrimaries));
    ...
    public void testUnallocatedShards() {
        ...
        final SnapshotException sne = expectThrows(
            ...
            clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, "test-repo", "test-snap")
                .setWaitForCompletion(true)
                .setIndices("test-idx")
        );
    ...
    public void testDeleteSnapshot() throws Exception {
        ...
        if (randomBoolean()) {
            for (int i = 1; i < numberOfSnapshots - 1; i++) {
                client.admin().cluster().prepareDeleteSnapshot(masterNodeTimeout, "test-repo", new String[] { "test-snap-" + i }).get();
            }
        } else {
            client.admin()
                .cluster()
        ...
        RestoreSnapshotResponse restoreSnapshotResponse = client.admin()
            .cluster()
            .prepareRestoreSnapshot(masterNodeTimeout, "test-repo", lastSnapshot)
            .setWaitForCompletion(true)
            .get();
        assertThat(restoreSnapshotResponse.getRestoreInfo().totalShards(), greaterThan(0));
    ...
    public void testSnapshotClosedIndex() throws Exception {
        ...
        assertAcked(client.admin().indices().prepareClose(masterNodeTimeout, "test-idx-closed"));
        ClusterStateResponse stateResponse = client.admin().cluster().prepareState(masterNodeTimeout).get();
    ...
    public void testDeleteRepositoryWhileSnapshotting() throws Exception {
        ...
        try {
            client.admin().cluster().prepareDeleteRepository(masterNodeTimeout, randomFrom("test-repo", "test-*", "*")).get();
        ...
        assertAcked(
            client.admin()
                .cluster()
                .preparePutRepository(masterNodeTimeout, "test-repo-2")
    ...
    public void testReadonlyRepository() throws Exception {
        ...
        RestoreSnapshotResponse restoreSnapshotResponse = client.admin()
            .cluster()
            .prepareRestoreSnapshot(masterNodeTimeout, "readonly-repo", "test-snap")
            .setWaitForCompletion(true)
            .setIndices("test-idx")
            .get();
        assertThat(restoreSnapshotResponse.getRestoreInfo().totalShards(), greaterThan(0));
        ...
        GetSnapshotsResponse getSnapshotsResponse = client.admin().cluster().prepareGetSnapshots(masterNodeTimeout, "readonly-repo").get();
        assertThat(getSnapshotsResponse.getSnapshots().size(), equalTo(1));

        logger.info("--> try deleting snapshot");
        assertRequestBuilderThrows(
            client.admin().cluster().prepareDeleteSnapshot(masterNodeTimeout, "readonly-repo", "test-snap"),
        ...
        assertRequestBuilderThrows(
            client.admin()
                .cluster()
                .prepareCreateSnapshot(masterNodeTimeout, "readonly-repo", "test-snap-2")
                .setWaitForCompletion(true)
                .setIndices("test-idx"),
            RepositoryException.class,
    ...
    public void testSnapshotStatus() throws Exception {
        ...
        SnapshotsStatusResponse response = client.admin().cluster().prepareSnapshotStatus(masterNodeTimeout, "test-repo").get();
        ...
        response = client.admin().cluster().prepareSnapshotStatus(masterNodeTimeout).get();
        assertThat(response.getSnapshots().size(), equalTo(1));
        ...
        snapshotStatus = client.admin().cluster().prepareSnapshotStatus(masterNodeTimeout).get().getSnapshots().get(0);
        ...
        response = client.admin().cluster().prepareSnapshotStatus(masterNodeTimeout, "test-repo").addSnapshots("test-snap").get();
        snapshotStatus = response.getSnapshots().get(0);
        assertThat(snapshotStatus.getIndices().size(), equalTo(1));
        assertThat(snapshotStatus.includeGlobalState(), equalTo(false));
        ...
        SnapshotMissingException ex = expectThrows(
            ...
            client.admin().cluster().prepareSnapshotStatus(masterNodeTimeout, "test-repo").addSnapshots("test-snap-doesnt-exist")
        );
        ...
        response = client.admin()
            .cluster()
            .prepareSnapshotStatus(masterNodeTimeout, "test-repo")
    ...
    public void testSnapshotRelocatingPrimary() throws Exception {
        ...
        assertBusy(
            () -> assertThat(clusterAdmin().prepareHealth(masterNodeTimeout, "test-idx").get().getRelocatingShards(), greaterThan(0)),
            1L,
            TimeUnit.MINUTES
        );
    ...
    public void testCloseOrDeleteIndexDuringSnapshot() throws Exception {
        ...
        logger.info("--> snapshot");
        ActionFuture<CreateSnapshotResponse> future = clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, "test-repo", "test-snap")
            .setIndices("test-idx-*")
            .setWaitForCompletion(true)
            .setPartial(false)
            .execute();
        ...
        try {
            ...
            if (randomBoolean()) {
                try {
                    ...
                    indicesAdmin().prepareDelete(masterNodeTimeout, "test-idx-1").get();
                ...
                try {
                    ...
                    indicesAdmin().prepareClose(masterNodeTimeout, "test-idx-1").get();
    ...
    public void testCloseIndexDuringRestore() throws Exception {
        ...
        try {
            ...
            try {
                client.admin().indices().prepareClose(masterNodeTimeout, "test-idx-1").get();
    ...
    public void testDeleteSnapshotWhileRestoringFails() throws Exception {
        ...
        assertThat(
            client.admin()
                .cluster()
                .prepareCreateSnapshot(masterNodeTimeout, repoName, snapshotName2)
                .setIndices(indexName)
                .setWaitForCompletion(true)
                .get()
                .getSnapshotInfo()
        ...
        assertAcked(client.admin().indices().prepareDelete(masterNodeTimeout, indexName).get());
        ...
        try {
            ...
            restoreFut = client.admin()
                .cluster()
                .prepareRestoreSnapshot(masterNodeTimeout, repoName, snapshotName)
                .setWaitForCompletion(true)
                .execute();
            ...
            ConcurrentSnapshotExecutionException e = expectThrows(
                ...
                clusterAdmin().prepareDeleteSnapshot(masterNodeTimeout, repoName, snapshotName)
            );
    ...
    public void testSnapshotName() throws Exception {
        ...
        expectThrows(
            ...
            client.admin().cluster().prepareCreateSnapshot(masterNodeTimeout, "test-repo", "_foo")
        );
        expectThrows(
            SnapshotMissingException.class,
            client.admin().cluster().prepareGetSnapshots(masterNodeTimeout, "test-repo").setSnapshots("_foo")
        );
        expectThrows(
            SnapshotMissingException.class,
            client.admin().cluster().prepareDeleteSnapshot(masterNodeTimeout, "test-repo", "_foo")
        );
        expectThrows(
            SnapshotMissingException.class,
            client.admin().cluster().prepareSnapshotStatus(masterNodeTimeout, "test-repo").setSnapshots("_foo")
        );
    }
    ...
    public void testListCorruptedSnapshot() throws Exception {
        ...
        final SnapshotException ex = expectThrows(
            SnapshotException.class,
            client.admin().cluster().prepareGetSnapshots(masterNodeTimeout, "test-repo").setIgnoreUnavailable(false)
        );
    ...
    public void testRestoreSnapshotWithCorruptedGlobalState() throws Exception {
        ...
        List<SnapshotInfo> snapshotInfos = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, repoName).get().getSnapshots();
        assertThat(snapshotInfos.size(), equalTo(1));
        assertThat(snapshotInfos.get(0).state(), equalTo(SnapshotState.SUCCESS));
        assertThat(snapshotInfos.get(0).snapshotId().getName(), equalTo(snapshotName));

        SnapshotsStatusResponse snapshotStatusResponse = clusterAdmin().prepareSnapshotStatus(masterNodeTimeout, repoName)
        ...
        assertAcked(indicesAdmin().prepareDelete(masterNodeTimeout, "test-idx-1", "test-idx-2"));
    ...
    public void testRestoreSnapshotWithCorruptedIndexMetadata() throws Exception {
        ...
        assertAcked(indicesAdmin().prepareDelete(masterNodeTimeout, nbDocsPerIndex.keySet().toArray(new String[nbDocsPerIndex.size()])));
    ...
    public void testCannotCreateSnapshotsWithSameName() throws Exception {
        ...
        try {
            CreateSnapshotResponse createSnapshotResponse = client.admin()
                .cluster()
                .prepareCreateSnapshot(masterNodeTimeout, repositoryName, snapshotName)
                .setWaitForCompletion(true)
                .setIndices(indexName)
                .get();
            fail(
    ...
    public void testGetSnapshotsFromIndexBlobOnly() throws Exception {
        ...
        GetSnapshotsResponse response = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
            .setSnapshots("_all")
            .setVerbose(false)
            .get();
        ...
        response = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo").setSnapshots("test-snap-*").setVerbose(false).get();
        assertEquals(indicesPerSnapshot.size(), response.getSnapshots().size());
        verifySnapshotInfo(response, indicesPerSnapshot);

        logger.info("--> verify individual requests return snapshot info");
        for (int i = 0; i < numSnapshots; i++) {
            response = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
    ...
    public void testSnapshotDifferentIndicesBySameName() throws InterruptedException, ExecutionException {
        ...
        clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repoName, "snap-1")
            .setIndices(indexName)
            .setRenamePattern(indexName)
        ...
        clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repoName, "snap-2")
            .setIndices(indexName)
            .setRenamePattern(indexName)
        ...
        clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repoName, snapshotToRestore)
            .setIndices(indexName)
            .setRenamePattern(indexName)
    ...
    public void testBulkDeleteWithOverlappingPatterns() {
        ...
        clusterAdmin().prepareDeleteSnapshot(masterNodeTimeout, "test-repo", "test-snap-*", "*").get();
        final GetSnapshotsResponse getSnapshotsResponse = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo").get();
    ...
    public void testHiddenIndicesIncludedInSnapshot() throws Exception {
        ...
        List<SnapshotInfo> snapshotInfos = client.admin()
            .cluster()
            .prepareGetSnapshots(masterNodeTimeout, repoName)
        ...
        {
            RestoreSnapshotResponse restoreSnapshotResponse = clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repoName, snapName)
                .setWaitForCompletion(true)
                .setIndices("*")
                .execute()
                .get();
            ...
            ClusterState clusterState = client.admin().cluster().prepareState(masterNodeTimeout).get().getState();
