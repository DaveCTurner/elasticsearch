...
public class BlobStoreIncrementalityIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testIncrementalBehaviorOnPrimaryFailover() throws InterruptedException, ExecutionException, IOException {
        ...
        clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, repo, snapshot3).setIndices(indexName).setWaitForCompletion(true).get();
        ...
        clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, repo, snapshot4).setIndices(indexName).setWaitForCompletion(true).get();
    ...
    public void testForceMergeCausesFullSnapshot() throws Exception {
        ...
        clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, repo, snapshot1).setIndices(indexName).setWaitForCompletion(true).get();
        ...
        final String snapshot2 = "snap-2";
        logger.info("--> creating snapshot 2");
        clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, repo, snapshot2).setIndices(indexName).setWaitForCompletion(true).get();
    ...
    private void ensureRestoreSingleShardSuccessfully(String repo, String indexName, String snapshot, String indexSuffix) {
        ...
        final RestoreSnapshotResponse restoreSnapshotResponse = clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, repo, snapshot)
