...
public class SnapshotStatusApisIT extends AbstractSnapshotIntegTestCase {
    ...
    public void testStatusApiConsistency() throws Exception {
        ...
        final List<SnapshotStatus> snapshotStatus = clusterAdmin().prepareSnapshotStatus(masterNodeTimeout, "test-repo")
    ...
    public void testExceptionOnMissingShardLevelSnapBlob() throws IOException {
        ...
        expectThrows(
            SnapshotMissingException.class,
            clusterAdmin().prepareSnapshotStatus(masterNodeTimeout, "test-repo").setSnapshots("test-snap")
        );
    }
    ...
    public void testGetSnapshotsWithoutIndices() throws Exception {
        ...
        final List<SnapshotInfo> snapshotInfos = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
            .setVerbose(false)
            .get()
            .getSnapshots();
    ...
    public void testCorrectCountsForDoneShards() throws Exception {
        ...
        final ActionFuture<CreateSnapshotResponse> responseSnapshotOne = internalCluster().masterClient()
            .admin()
            .cluster()
            .prepareCreateSnapshot(masterNodeTimeout, repoName, snapshotOne)
            .setWaitForCompletion(true)
            .execute();
    ...
    public void testGetSnapshotsNoRepos() {
        ensureGreen();
        GetSnapshotsResponse getSnapshotsResponse = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, new String[] { "_all" })
    ...
    public void testGetSnapshotsMultipleRepos() throws Exception {
        ...
        final int numberOfShards = IndexMetadata.INDEX_NUMBER_OF_SHARDS_SETTING.get(
            client.admin().indices().prepareGetSettings(masterNodeTimeout, indexName).get().getIndexToSettings().get(indexName)
        );
        ...
        GetSnapshotsResponse getSnapshotsResponse2 = client.admin()
            .cluster()
            .prepareGetSnapshots(masterNodeTimeout, randomFrom("_all", "repo*"))
    ...
    public void testGetSnapshotsWithSnapshotInProgress() throws Exception {
        ...
        GetSnapshotsResponse response1 = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
            .setSnapshots("test-snap")
            .setIgnoreUnavailable(true)
            .get();
        ...
        SnapshotStatus snapshotStatus = clusterAdmin().prepareSnapshotStatus(masterNodeTimeout).get().getSnapshots().get(0);
        ...
        GetSnapshotsResponse response2 = clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
        ...
        expectThrows(
            SnapshotMissingException.class,
            clusterAdmin().prepareGetSnapshots(masterNodeTimeout, "test-repo")
                .setSnapshots(notExistedSnapshotName)
                .setIgnoreUnavailable(false)
        );
    ...
    public void testSnapshotStatusOnFailedSnapshot() throws Exception {
        ...
        final SnapshotsStatusResponse snapshotsStatusResponse = clusterAdmin().prepareSnapshotStatus(masterNodeTimeout, repoName)
    ...
    public void testGetSnapshotsRequest() throws Exception {
        ...
        expectThrows(
            SnapshotMissingException.class,
            client.admin().cluster().prepareGetSnapshots(masterNodeTimeout, repositoryName).addSnapshots("non-existent-snapshot")
        );
        ...
        client.admin()
            .cluster()
            .prepareCreateSnapshot(masterNodeTimeout, repositoryName, "snap-on-empty-repo")
        ...
        client.admin()
            .cluster()
            .prepareCreateSnapshot(masterNodeTimeout, repositoryName, inProgressSnapshot)
            .setWaitForCompletion(false)
            .setIndices(indexName)
            .get();
    ...
    public void testConcurrentCreateAndStatusAPICalls() throws Exception {
        ...
        for (int i = 0; i < metaThreadPoolSize * 2; i++) {
            statuses.add(
                dataNodeClient.admin().cluster().prepareSnapshotStatus(masterNodeTimeout, repoName).setSnapshots(snapshotNames).execute()
            );
            gets.add(
                dataNodeClient.admin().cluster().prepareGetSnapshots(masterNodeTimeout, repoName).setSnapshots(snapshotNames).execute()
            );
        }
    ...
    private static SnapshotStatus getSnapshotStatus(String repoName, String snapshotName) {
        try {
            return clusterAdmin().prepareSnapshotStatus(masterNodeTimeout, repoName).setSnapshots(snapshotName).get().getSnapshots().get(0);
