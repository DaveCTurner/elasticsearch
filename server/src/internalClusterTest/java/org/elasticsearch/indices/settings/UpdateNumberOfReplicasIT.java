...
public class UpdateNumberOfReplicasIT extends ESIntegTestCase {
    ...
    public void testSimpleUpdateNumberOfReplicas() throws Exception {
        ...
        logger.info("Running Cluster Health");
        clusterHealth = clusterAdmin().prepareHealth(masterNodeTimeout)
            .setWaitForEvents(Priority.LANGUID)
        ...
        final long afterReplicaIncreaseSettingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .metadata()
            .index("test")
        ...
        final long afterStartingAnotherNodeVersion = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .metadata()
            .index("test")
        ...
        final long afterReplicaDecreaseSettingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .metadata()
            .index("test")
    ...
    public void testAutoExpandNumberOfReplicas0ToData() throws IOException {
        ...
        if (randomBoolean()) {
            assertAcked(indicesAdmin().prepareClose(masterNodeTimeout, "test").setWaitForActiveShards(ActiveShardCount.ALL));
        ...
        final long afterAddingOneNodeSettingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .metadata()
            .index("test")
        ...
        final long afterClosingOneNodeSettingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .metadata()
            .index("test")
        ...
        final long afterClosingAnotherNodeSettingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
            .get()
            .getState()
            .metadata()
            .index("test")
    ...
    public void testAutoExpandNumberReplicas2() {
         ...
         */
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index("test").getSettingsVersion(),
    ...
    public void testUpdateNumberOfReplicasAllowNoIndices() {
        ...
        final int numberOfReplicas = Integer.parseInt(
            indicesAdmin().prepareGetSettings(masterNodeTimeout, "test-index").get().getSetting("test-index", "index.number_of_replicas")
        );
