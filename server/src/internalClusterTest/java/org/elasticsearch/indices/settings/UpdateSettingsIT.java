...
public class UpdateSettingsIT extends ESIntegTestCase {
    ...
    public void testInvalidDynamicUpdate() {
        ...
        IllegalArgumentException exception = expectThrows(
            IllegalArgumentException.class,
            indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(Settings.builder().put("index.dummy", "boom"))
        );
        assertEquals(exception.getCause().getMessage(), "this setting goes boom");
        IndexMetadata indexMetadata = clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index("test");
    ...
    public void testResetDefaultWithWildcard() {
        ...
        indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(Settings.builder().putNull("index.ref*")).get();
        indexMetadata = clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index("test");
    ...
    public void testOpenCloseUpdateSettings() throws Exception {
        ...
        // Now verify via dedicated get settings api:
        getSettingsResponse = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test").get();
        ...
        ClusterHealthResponse health = clusterAdmin().prepareHealth(masterNodeTimeout)
    ...
    public void testEngineGCDeletesSetting() throws Exception {
        ...
        assertAcked(
            indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(Settings.builder().put("index.gc_deletes", 0))
        );
    ...
    public void testUpdateSettingsWithBlocks() {
        ...
        for (String blockSetting : Arrays.asList(SETTING_BLOCKS_READ, SETTING_BLOCKS_WRITE)) {
            try {
                ...
                assertAcked(indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(builder));
            } finally {
        ...
        for (String blockSetting : Arrays.asList(SETTING_READ_ONLY, SETTING_BLOCKS_METADATA)) {
            try {
                enableIndexBlock("test", blockSetting);
                assertBlocked(indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(builder));
            } finally {
                disableIndexBlock("test", blockSetting);
            }
        }
    ...
    public void testSettingsVersion() {
        ...
        {
            final long settingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
                .get()
                .getState()
                .metadata()
                .index("test")
            ...
            final long newSettingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
                .get()
                .getState()
                .metadata()
                .index("test")
    ...
    public void testSettingsVersionUnchanged() {
...

        {
            final long settingsVersion = clusterAdmin().prepareState(masterNodeTimeout)
                ...
                .index("test")
                .getSettingsVersion();
            final String refreshInterval = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test")
                .get()
    ...
    public void testNumberOfReplicasSettingsVersionUnchanged() {
        ...
        final int numberOfReplicas = Integer.valueOf(
            indicesAdmin().prepareGetSettings(masterNodeTimeout, "test").get().getSetting("test", "index.number_of_replicas")
        );
        assertAcked(
            indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test")
    ...
    private void runTestDefaultNumberOfReplicasTest(final boolean closeIndex) {
        ...
        final GetSettingsResponse response = indicesAdmin().prepareGetSettings(masterNodeTimeout, "test").get();
    ...
    public void testNoopUpdate() {
        ...
        assertAcked(
            indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(Settings.builder().put(SETTING_BLOCKS_READ, true))
        );
        assertSame(currentState, clusterService.state());

        assertAcked(
            indicesAdmin().prepareUpdateSettings(masterNodeTimeout, "test").setSettings(Settings.builder().putNull(SETTING_BLOCKS_READ))
        );
        assertNotSame(currentState, clusterService.state());
    }
    ...
    public void testAllSettingStringInterned() {
        ...
        assertAcked(
            indicesAdmin().prepareUpdateSettings(masterNodeTimeout, index1, index2)
