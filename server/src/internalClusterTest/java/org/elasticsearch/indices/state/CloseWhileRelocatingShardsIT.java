...
public class CloseWhileRelocatingShardsIT extends ESIntegTestCase {
    ...
    public void testCloseWhileRelocatingShards() throws Exception {
        ...
        try {
            ...
            assertAcked(clusterAdmin().reroute(new ClusterRerouteRequest(masterNodeTimeout).commands(commands)).get());
            ...
            for (final String indexToClose : indices) {
                final Thread thread = new Thread(() -> {
                    ...
                    AcknowledgedResponse closeResponse = indicesAdmin().prepareClose(masterNodeTimeout, indexToClose).get();
