...
public class ReplicaToPrimaryPromotionIT extends ESIntegTestCase {
    ...
    public void testPromoteReplicaToPrimary() throws Exception {
        ...
        ClusterState state = client(internalCluster().getMasterName()).admin().cluster().prepareState(masterNodeTimeout).get().getState();
        ...
        state = client(internalCluster().getMasterName()).admin().cluster().prepareState(masterNodeTimeout).get().getState();
