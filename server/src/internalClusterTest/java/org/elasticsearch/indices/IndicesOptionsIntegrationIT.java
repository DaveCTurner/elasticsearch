...
public class IndicesOptionsIntegrationIT extends ESIntegTestCase {
    ...
    public void testSpecifiedIndexUnavailableSnapshotRestore() throws Exception {
        ...
        AcknowledgedResponse putRepositoryResponse = clusterAdmin().preparePutRepository(masterNodeTimeout, "dummy-repo")
            .setType("fs")
            .setSettings(Settings.builder().put("location", randomRepoPath()))
            .get();
        assertThat(putRepositoryResponse.isAcknowledged(), equalTo(true));
        clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, "dummy-repo", "snap1").setWaitForCompletion(true).get();
    ...
    public void testUpdateSettings() throws Exception {
        ...
        GetSettingsResponse settingsResponse = indicesAdmin().prepareGetSettings(masterNodeTimeout, "foo").get();
        assertThat(settingsResponse.getSetting("foo", "index.a"), equalTo("b"));
        settingsResponse = indicesAdmin().prepareGetSettings(masterNodeTimeout, "bar*").get();
        ...
        settingsResponse = indicesAdmin().prepareGetSettings(masterNodeTimeout, "_all").get();
    ...
    static GetSettingsRequestBuilder getSettings(String... indices) {
        return indicesAdmin().prepareGetSettings(masterNodeTimeout, indices);
    }

    private static CreateSnapshotRequestBuilder snapshot(String name, String... indices) {
        return clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, "dummy-repo", name).setWaitForCompletion(true).setIndices(indices);
    }

    private static RestoreSnapshotRequestBuilder restore(String name, String... indices) {
        return clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, "dummy-repo", name)
            .setRenamePattern("(.+)")
