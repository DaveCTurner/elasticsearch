...
public class GetIndexIT extends ESIntegTestCase {
    ...
    public void testSimple() {
        GetIndexResponse response = indicesAdmin().prepareGetIndex(masterNodeTimeout).addIndices("idx").get();
    ...
    public void testSimpleUnknownIndex() {
        try {
            indicesAdmin().prepareGetIndex(masterNodeTimeout).addIndices("missing_idx").get();
    ...
    public void testUnknownIndexWithAllowNoIndices() {
        GetIndexResponse response = indicesAdmin().prepareGetIndex(masterNodeTimeout)
    ...
    public void testEmpty() {
        GetIndexResponse response = indicesAdmin().prepareGetIndex(masterNodeTimeout).addIndices("empty_idx").get();
        String[] indices = response.indices();
        assertThat(indices, notNullValue());
        assertThat(indices.length, equalTo(1));
    ...
    public void testSimpleMapping() {
        GetIndexResponse response = runWithRandomFeatureMethod(
            indicesAdmin().prepareGetIndex(masterNodeTimeout).addIndices("idx"),
            Feature.MAPPINGS
        );
        String[] indices = response.indices();
        assertThat(indices, notNullValue());
    ...
    public void testEmptyMixedFeatures() {
        ...
        GetIndexResponse response = runWithRandomFeatureMethod(
            indicesAdmin().prepareGetIndex(masterNodeTimeout).addIndices("empty_idx"),
            features.toArray(new Feature[features.size()])
        );
        String[] indices = response.indices();
        assertThat(indices, notNullValue());
    ...
    public void testGetIndexWithBlocks() {
        ...
        try {
            ...
            assertBlocked(
                indicesAdmin().prepareGetIndex(masterNodeTimeout).addIndices("idx").addFeatures(Feature.MAPPINGS, Feature.ALIASES),
