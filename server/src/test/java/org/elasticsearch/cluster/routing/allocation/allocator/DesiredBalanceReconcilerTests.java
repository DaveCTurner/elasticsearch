...
public class DesiredBalanceReconcilerTests extends ESTestCase {
    ...
    public void testUnassignedShardsInterleaving() {
        // regardless of priority, we give each shard an opportunity to allocate one of its replicas before we give any shard an opportunity
        // to allocate a further replica
    ...
    public void testUnassignedSkipsEquivalentReplicas() {
        ...
        final var existingIndexMetadata = randomPriorityIndex("index-0", 1, between(0, 5));
        metadata.put(existingIndexMetadata, true);
        routingTable.addAsNew(existingIndexMetadata);
