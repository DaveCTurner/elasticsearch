...
public class TestTranslog {
    ...
    static void corruptFile(Logger logger, Random random, Path fileToCorrupt, boolean maybeDelete) throws IOException {
        ...
        if (maybeDelete && (fileSize == 0 || RandomNumbers.randomIntBetween(random, 0, 9) == 0)) {
            logger.info("--> deleting {}", fileToCorrupt);
            Files.delete(fileToCorrupt);
            return;
        }

        assertThat("cannot corrupt empty file " + fileToCorrupt, fileSize, greaterThan(0L));

        logger.info("--> corrupting {}", fileToCorrupt);
        ...
        try (FileChannel fileChannel = FileChannel.open(fileToCorrupt, StandardOpenOption.READ, StandardOpenOption.WRITE)) {
            ...
            if (random.nextBoolean()) {
                ...
                // corrupt
                final String corruptionDescription;
                byte oldValue = bb.get(0);
                byte newValue;
                if (random.nextBoolean()) {
                    final int flippedBit = random.nextInt(8);
                    corruptionDescription = "flipping bit " + flippedBit;
                    newValue = (byte) (oldValue ^ (1 << flippedBit));
                } else if (random.nextInt(10) > 0) {
                    if (oldValue == ((byte) 0xff) || (random.nextBoolean() && oldValue != ((byte) 0x00))) {
                        newValue = (byte) 0x00;
                        corruptionDescription = "clearing all bits";
                    } else {
                        newValue = (byte) 0xff;
                        corruptionDescription = "setting all bits";
                    }
                } else {
                    newValue = (byte) random.nextInt(0x100);
                    corruptionDescription = "randomizing";
                }
                bb.put(0, newValue);
                ...
                logger.info("--> corrupting file {} -- {} at position {} turning 0x{} into 0x{}", fileToCorrupt, corruptionDescription,
                    corruptPosition, Integer.toHexString(oldValue & 0xff), Integer.toHexString(newValue & 0xff));
            } else {
                logger.info("--> corrupting file {} -- truncating from length {} to length {}", fileToCorrupt, fileSize, corruptPosition);
