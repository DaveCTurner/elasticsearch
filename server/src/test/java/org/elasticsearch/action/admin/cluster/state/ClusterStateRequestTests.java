...
public class ClusterStateRequestTests extends ESTestCase {
    public void testSerialization() throws Exception {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            ClusterStateRequest clusterStateRequest = new ClusterStateRequest(masterNodeTimeout).routingTable(randomBoolean())
    ...
    public void testDescription() {
        assertThat(new ClusterStateRequest(masterNodeTimeout).clear().getDescription(), equalTo("cluster state [master timeout [30s]]"));
        assertThat(
            new ClusterStateRequest(masterNodeTimeout).masterNodeTimeout(TimeValue.timeValueMinutes(5)).getDescription(),
            equalTo("cluster state [routing table, nodes, metadata, blocks, customs, master timeout [5m]]")
        );
        assertThat(new ClusterStateRequest(masterNodeTimeout).clear().routingTable(true).getDescription(), containsString("routing table"));
        assertThat(new ClusterStateRequest(masterNodeTimeout).clear().nodes(true).getDescription(), containsString("nodes"));
        assertThat(new ClusterStateRequest(masterNodeTimeout).clear().metadata(true).getDescription(), containsString("metadata"));
        assertThat(new ClusterStateRequest(masterNodeTimeout).clear().blocks(true).getDescription(), containsString("blocks"));
        assertThat(new ClusterStateRequest(masterNodeTimeout).clear().customs(true).getDescription(), containsString("customs"));
        assertThat(new ClusterStateRequest(masterNodeTimeout).local(true).getDescription(), containsString("local"));
        assertThat(
            new ClusterStateRequest(masterNodeTimeout).waitForMetadataVersion(23L).getDescription(),
            containsString("wait for metadata version [23] with timeout [1m]")
        );
        assertThat(new ClusterStateRequest(masterNodeTimeout).indices("foo", "bar").getDescription(), containsString("indices [foo, bar]"));
    }
}
