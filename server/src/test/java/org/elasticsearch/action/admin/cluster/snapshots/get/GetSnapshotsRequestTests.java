...
public class GetSnapshotsRequestTests extends ESTestCase {
    ...
    public void testValidateParameters() {
        {
            final GetSnapshotsRequest request = new GetSnapshotsRequest(masterNodeTimeout, "repo", "snapshot");
        ...
        {
            final GetSnapshotsRequest request = new GetSnapshotsRequest(masterNodeTimeout, "repo", "snapshot").size(
                randomIntBetween(1, 500)
            );
            assertNull(request.validate());
        }
        {
            final GetSnapshotsRequest request = new GetSnapshotsRequest(masterNodeTimeout, "repo", "snapshot").verbose(false)
        ...
        {
            final GetSnapshotsRequest request = new GetSnapshotsRequest(masterNodeTimeout, "repo", "snapshot").after(
        ...
        {
            final GetSnapshotsRequest request = new GetSnapshotsRequest(masterNodeTimeout, "repo", "snapshot").fromSortValue("foo")
                .after(new SnapshotSortKey.After("foo", "repo", "bar"));
            final ActionRequestValidationException e = request.validate();
            assertThat(e.getMessage(), containsString("can't use after and from_sort_value simultaneously"));
        }
        {
            final GetSnapshotsRequest request = new GetSnapshotsRequest(masterNodeTimeout, "repo", "snapshot").policies("some-policy")
                .verbose(false);
            final ActionRequestValidationException e = request.validate();
