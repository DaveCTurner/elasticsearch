...
public class RolloverIT extends ESIntegTestCase {
    ...
    public void testRolloverMaxSize() throws Exception {
        ...
        // A large max_size
        {
            ...
            final IndexMetaData oldIndex = client().admin().cluster().prepareState().setCompressedClusterStateSize(false)
                .get().getState().metaData().index("test-1");
        ...
        // An empty index
        {
            ...
            final IndexMetaData oldIndex = client().admin().cluster().prepareState().setCompressedClusterStateSize(false)
                .get().getState().metaData().index("test-000002");
            assertThat(oldIndex.getRolloverInfos().size(), equalTo(0));
        }
    }
