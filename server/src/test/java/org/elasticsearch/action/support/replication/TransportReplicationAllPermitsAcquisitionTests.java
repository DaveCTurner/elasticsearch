...
public class TransportReplicationAllPermitsAcquisitionTests extends IndexShardTestCase {
        ...
        MockTransport transport = new MockTransport() {
            ...
            protected void onSendRequest(long requestId, String action, TransportRequest request, DiscoveryNode node) {
                ...
                transportService.sendRequest(transportService.getLocalNode(), action, request,
                    new TransportResponseHandler<TransportRerouteFreeReplicationAction.ReplicaResponse>() {
                        @Override
                        public TransportRerouteFreeReplicationAction.ReplicaResponse read(StreamInput in) throws IOException {
                            final TransportRerouteFreeReplicationAction.ReplicaResponse replicaResponse
                                = new TransportRerouteFreeReplicationAction.ReplicaResponse();
                            replicaResponse.readFrom(in);
                            return replicaResponse;
                        }

                        @SuppressWarnings("unchecked")
                        private TransportResponseHandler<TransportRerouteFreeReplicationAction.ReplicaResponse> getResponseHandler() {
                            return (TransportResponseHandler<TransportRerouteFreeReplicationAction.ReplicaResponse>)
                                getResponseHandlers().onResponseReceived(requestId, TransportMessageListener.NOOP_LISTENER);
                        }

                        @Override
                        public void handleResponse(TransportRerouteFreeReplicationAction.ReplicaResponse response) {
    ...
    private class AllPermitsThenBlockAction extends TestAction {
        ...
        @Override
        protected TransportRerouteFreeReplicationAction.ReplicaResult shardOperationOnReplica(Request shardRequest, IndexShard shard)
            throws Exception {
