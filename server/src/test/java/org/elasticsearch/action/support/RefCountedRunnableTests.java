...
public class RefCountedRunnableTests extends ESTestCase {
    ...
    public void testBasicOperation() throws InterruptedException {
        ...
        try (var refs = new RefCountedRunnable(new Runnable() {
            @Override
            public void run() {
    ...
    public void testAsyncAcquire() throws InterruptedException {
        ...
        try (var refs = new RefCountedRunnable(() -> {
    ...
    public void testValidation() {
        ...
        final var refs = new RefCountedRunnable(callCount::incrementAndGet);
        refs.close();
        assertEquals(1, callCount.get());

        for (int i = between(1, 5); i > 0; i--) {
            assertEquals(
                RefCountedRunnable.ALREADY_CLOSED_MESSAGE,
                expectThrows(AssertionError.class, randomBoolean() ? refs::acquire : refs::close).getMessage()
            );
            assertEquals(1, callCount.get());
        }
    }
