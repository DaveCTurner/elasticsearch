...
public class LifecycleTest extends ESTestCase {
    ...
    private static class ThreadSafetyTestHarness implements Releasable {
        ...
        final EsThreadPoolExecutor executor;
            ...
            PlainActionFuture.<Void, RuntimeException>get(fut -> {
                try (var listeners = new RefCountingListener(fut)) {
                    for (int i = 0; i < this.threads; i++) {
                        this.executor.submit(ActionRunnable.run(listeners.acquire(), () -> {
                            safeAwait(this.barrier);
