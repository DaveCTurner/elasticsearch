...
public class IteratorsTests extends ESTestCase {
    ...
    public void testFlatMap() {
        ...
        assertEmptyIterator(Iterators.flatMap(i -> Iterators.concat(), Iterators.forArray(array)));
        assertEmptyIterator(Iterators.flatMap(i -> Iterators.single("foo"), Iterators.concat()));

        final var index = new AtomicInteger();
        Iterators.flatMap(Iterators::single, Iterators.forArray(array))
            .forEachRemaining(i -> assertEquals(array[index.getAndIncrement()], i));
        assertEquals(array.length, index.get());

        index.set(0);
        Iterators.flatMap(i -> Iterators.forArray(new Integer[] { i, i, i }), Iterators.forArray(array))
        ...
        Iterators.flatMap(i -> IntStream.range(0, (int) i).iterator(), input.listIterator())
