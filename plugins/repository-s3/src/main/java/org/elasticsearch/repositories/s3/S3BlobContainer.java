...
class S3BlobContainer extends AbstractBlobContainer {
    ...
    private void deletePartition(AmazonS3Reference clientReference, List<String> partition, AtomicReference<Exception> aex) {
        try {
            final List<String> realBlobNames = partition.stream().map(s -> {
            ...
            logger.info("--> deletePartition{}", realBlobNames);
            clientReference.client().deleteObjects(bulkDelete(blobStore.bucket(), realBlobNames));
