...
public class OldRepositoryAccessIT extends ESRestTestCase {
    ...
    private void beforeRestart(
        ...
        createRepo.setJsonEntity(
            Strings.toString(
                new PutRepositoryRequest(masterNodeTimeout).type(sourceOnlyRepository ? "source" : "fs")
    ...
    private void restoreMountAndVerify(
        ...
        restoreRequest.setJsonEntity(
            Strings.toString(
                new RestoreSnapshotRequest(masterNodeTimeout).indices(indexName).renamePattern("(.+)").renameReplacement("restored_$1")
            )
        );
