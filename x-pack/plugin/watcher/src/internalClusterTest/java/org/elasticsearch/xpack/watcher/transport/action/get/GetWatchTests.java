...
public class GetWatchTests extends AbstractWatcherIntegrationTestCase {
    ...
    public void testGetNotFound() throws Exception {
        ...
        if (randomBoolean()) {
            try {
                GetIndexResponse indexResponse = indicesAdmin().prepareGetIndex(masterNodeTimeout).setIndices(Watch.INDEX).get();
                boolean isWatchIndexAlias = Watch.INDEX.equals(indexResponse.indices()[0]) == false;
                if (isWatchIndexAlias) {
                    assertAcked(indicesAdmin().prepareAliases(masterNodeTimeout).removeAlias(indexResponse.indices()[0], Watch.INDEX));
                } else {
                    assertAcked(indicesAdmin().prepareDelete(masterNodeTimeout, Watch.INDEX));
                }
