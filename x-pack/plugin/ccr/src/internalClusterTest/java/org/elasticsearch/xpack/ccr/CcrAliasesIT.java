...
public class CcrAliasesIT extends CcrIntegTestCase {
    ...
    private void addRandomAlias(final String index, final String aliasName, final Boolean isWriteIndex) {
        ...
        assertAcked(leaderClient().admin().indices().prepareAliases(masterNodeTimeout).addAliasAction(add));
    }
    ...
    public void testAddMultipleAliasesAtOnce() throws Exception {
        ...
        final IndicesAliasesRequestBuilder builder = leaderClient().admin().indices().prepareAliases(masterNodeTimeout);
    ...
    public void testAddMultipleAliasesSequentially() throws Exception {
        ...
        final int numberOfAliases = randomIntBetween(2, 8);
        for (int i = 0; i < numberOfAliases; i++) {
            assertAcked(leaderClient().admin().indices().prepareAliases(masterNodeTimeout).addAlias("leader", "alias_" + i));
    ...
    private void removeAlias(final String aliasName) {
        assertAcked(leaderClient().admin().indices().prepareAliases(masterNodeTimeout).removeAlias("leader", aliasName));
    }
    ...
    private String[] getAliasesOnLeader() throws InterruptedException, ExecutionException {
        final GetAliasesResponse response = leaderClient().admin()
            .indices()
            .getAliases(new GetAliasesRequest(masterNodeTimeout).indices("leader"))
            .get();
    ...
    private <E extends Exception> void assertAliasesExist(
        ...
        assertBusy(() -> {
            final GetAliasesResponse followerResponse = followerClient().admin()
                .indices()
                .getAliases(new GetAliasesRequest(masterNodeTimeout).indices(followerIndex))
                .get();
            assertThat(
            ...
            for (final String alias : aliases) {
                ...
                final GetAliasesResponse leaderResponse = leaderClient().admin()
                    .indices()
                    .getAliases(new GetAliasesRequest(masterNodeTimeout).indices(leaderIndex).aliases(alias))
                    .get();
    ...
    private void assertAliasExistence(final String alias, final boolean exists) throws Exception {
        assertBusy(() -> {
            ...
            final GetAliasesResponse response = followerClient().admin()
                .indices()
                .getAliases(new GetAliasesRequest(masterNodeTimeout).indices("follower").aliases(alias))
                .get();
            if (exists) {
