...
public class SearchableSnapshotsIntegTests extends BaseSearchableSnapshotsIntegTestCase {
    ...
    public void testCreateAndRestoreSearchableSnapshot() throws Exception {
        ...
        final Settings settings = indicesAdmin().prepareGetSettings(masterNodeTimeout, restoredIndexName)
            .get()
            .getIndexToSettings()
        ...
        final Settings clonedIndexSettings = indicesAdmin().prepareGetSettings(masterNodeTimeout, clonedIndexName)
            .get()
            .getIndexToSettings()
            .get(clonedIndexName);
    ...
    public void testSnapshotMountedIndexWithTimestampsRecordsTimestampRangeInIndexMetadata() throws Exception {
        ...
        final IndexLongFieldRange timestampRange = clusterAdmin().prepareState(masterNodeTimeout)
            .clear()
            .setMetadata(true)
            .setIndices(indexName)
            .get()
    ...
    public void testSnapshotOfSearchableSnapshotIncludesNoDataButCanBeRestored() throws Exception {
        ...
        if (hasRepositoryUuid) {
            ...
            assertAcked(clusterAdmin().prepareDeleteRepository(masterNodeTimeout, tmpRepositoryName));
        ...
        if (randomBoolean()) {
            ...
            assertAcked(indicesAdmin().prepareClose(masterNodeTimeout, restoredIndexName));
        }
        ...
        if (hasRepositoryUuid && randomBoolean()) {
            ...
            assertAcked(
                clusterAdmin().preparePutRepository(masterNodeTimeout, newRepositoryName)
                    .setType("fs")
        ...
        assertThat(
            clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, restoreRepositoryName, snapshotTwo.getName())
                .setIndices(restoredIndexName)
                .get()
                .status(),
    ...
    public void testSnapshotOfSearchableSnapshotCanBeRestoredBeforeRepositoryRegistered() throws Exception {
        ...
        assertThat(
            clusterAdmin().prepareRestoreSnapshot(masterNodeTimeout, backupRepoName, backupSnapshot.getName())
                .setIndices(restoredIndexName)
                .get()
                .status(),
            equalTo(RestStatus.ACCEPTED)
        ...
        assertBusy(() -> {
            final RestoreInProgress restoreInProgress = RestoreInProgress.get(
                clusterAdmin().prepareState(masterNodeTimeout).clear().setCustoms(true).get().getState()
            );
        ...
        assertAcked(clusterAdmin().preparePutRepository(masterNodeTimeout, newRepositoryName).setType("fs").setSettings(settings));
    ...
    public void testCheckOnStartupCanBeOverridden() throws Exception {
...

        {
            ...
            assertThat(
                indicesAdmin().prepareGetSettings(masterNodeTimeout, mountedIndex)
                    .get()
                    .getIndexToSettings()
                    .get(mountedIndex)
