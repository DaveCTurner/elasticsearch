...
public class SearchableSnapshotDiskThresholdIntegTests extends DiskUsageIntegTestCase {
    ...
    private void createRepository(String name, String type) {
        assertAcked(
            clusterAdmin().preparePutRepository(masterNodeTimeout, name)
                .setType(type)
    ...
    private void createSnapshot(String repository, String snapshot, int nbIndices) {
        var snapshotInfo = clusterAdmin().prepareCreateSnapshot(masterNodeTimeout, repository, snapshot)
            .setIndices("index-*")
    ...
    public void testHighWatermarkCanNotBeExceededOnColdNode() throws Exception {
        ...
        assertBusy(() -> {
            var state = clusterAdmin().prepareState(masterNodeTimeout).setRoutingTable(true).get().getState();
            assertThat(
                state.routingTable()
                    .allShards()
    ...
    public void testHighWatermarkCanNotBeExceededWithInitializingSearchableSnapshots() throws Exception {
        ...
        String coldNodeId = clusterAdmin().prepareState(masterNodeTimeout).get().getState().nodes().resolveNode(coldNodeName).getId();
