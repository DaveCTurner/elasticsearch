...
public class TransportMonitoringMigrateAlertsActionTests extends MonitoringIntegTestCase {
    ...
    private void checkMonitoringTemplates() {
        ...
        GetIndexTemplatesResponse response = client().admin().indices().prepareGetTemplates(masterNodeTimeout, ".monitoring-*").get();
    ...
    private void assertWatchesExist(boolean exist) {
        ...
        if (client().admin().indices().prepareGetIndex(masterNodeTimeout).addIndices(".watches").get().getIndices().length == 0) {
