...
public class IndexAliasesTests extends SecurityIntegTestCase {
    ...
    public void testCreateIndexThenAliasesCreateOnlyPermission() {
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_*", "test_alias")::get,
            TransportIndicesAliasesAction.NAME,
            "create_only"
        );
    }
    ...
    public void testDeleteAliasesCreateOnlyPermission() {
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "alias_1")::get,
            TransportIndicesAliasesAction.NAME,
            "create_only"
        );

        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "alias_*")::get,
            TransportIndicesAliasesAction.NAME,
            "create_only"
        );

        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "_all")::get,
            TransportIndicesAliasesAction.NAME,
            "create_only"
        );
    }
    ...
    public void testGetAliasesCreateOnlyPermissionStrict() {
        ...
        assertThrowsAuthorizationException(
            client.admin()
                .indices()
                .prepareGetAliases(masterNodeTimeout, "test_1")
                .setIndices("test_1")
        ...
        assertThrowsAuthorizationException(
            client.admin()
                .indices()
                .prepareGetAliases(masterNodeTimeout, "_all")
                .setIndices("test_1")
                .setIndicesOptions(IndicesOptions.strictExpand())::get,
            GetAliasesAction.NAME,
            "create_only"
        ...
        assertThrowsAuthorizationException(
            client.admin()
                .indices()
                .prepareGetAliases(masterNodeTimeout)
                .setIndices("test_1")
                .setIndicesOptions(IndicesOptions.strictExpand())::get,
            GetAliasesAction.NAME,
            "create_only"
        ...
        assertThrowsAuthorizationException(
            client.admin()
                .indices()
                .prepareGetAliases(masterNodeTimeout, "test_alias")
                .setIndices("test_*")
                .setIndicesOptions(IndicesOptions.strictExpand())::get,
            GetAliasesAction.NAME,
            "create_only"
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareGetAliases(masterNodeTimeout)::get,
            GetAliasesAction.NAME,
            "create_only"
        );
    }
    ...
    public void testGetAliasesCreateOnlyPermissionIgnoreUnavailable() {
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareGetAliases(masterNodeTimeout, "alias*")::get,
            GetAliasesAction.NAME,
            "create_only"
        );
        ...
        // and there is no way to "allow_no_aliases" like we can do with indices.
        assertThrowsAuthorizationException(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setIndicesOptions(IndicesOptions.lenientExpandOpen())::get,
            GetAliasesAction.NAME,
            "create_only"
        );
    }
    ...
    public void testCreateIndexThenAliasesCreateAndAliasesPermission() {
        ...
        // ok: user has manage_aliases on test_*
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_*", "test_alias_2").get());

        // fails: user doesn't have manage_aliases on alias_1
        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_1", "alias_1").addAlias("test_1", "test_alias")::get,
            TransportIndicesAliasesAction.NAME,
            "create_test_aliases_test"
        );
    }
    ...
    public void testDeleteAliasesCreateAndAliasesPermission() {
        ...
        // ok: user has manage_aliases on test_*
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "test_alias_1").get());
        // ok: user has manage_aliases on test_*
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_*", "test_alias_2").get());
        // ok: user has manage_aliases on test_*
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "test_alias_*").get());

        {
            ...
            AliasesNotFoundException exception = expectThrows(
                ...
                client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "test_alias_*")::get
            );
        ...
        if (randomBoolean()) {
            assertAcked(indicesAdmin().prepareAliases(masterNodeTimeout).addAlias("test_1", "alias_1").get());
        }
        assertAcked(indicesAdmin().prepareAliases(masterNodeTimeout).addAlias("test_1", "alias_2").get());
        ...
        // fails: user doesn't have manage_aliases on alias_1
        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", new String[] { "_all", "alias_1" })::get,
            TransportIndicesAliasesAction.NAME,
            "create_test_aliases_test"
        );
        ...
        AliasesNotFoundException exception = expectThrows(
            AliasesNotFoundException.class,
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "*")::get
        );
    ...
    public void testGetAliasesCreateAndAliasesPermission() {
        ...
        // ok: user has manage_aliases on test_*
        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("test_alias").setIndices("test_1"),
            "test_1",
            "test_alias"
        );
        ...
        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("test_alias").setIndices("test_*"),
            "test_1",
            "test_alias"
        );

        // ok: user has manage_aliases on test_*, empty indices gets resolved to _all indices (thus test_1)
        assertAliases(client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("test_alias"), "test_1", "test_alias");

        // ok: user has manage_aliases on test_*, _all aliases gets resolved to test_alias and empty indices gets resolved to _all
        // indices (thus test_1)
        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("_all").setIndices("test_1"),
            "test_1",
            "test_alias"
        );
        ...
        // indices (thus test_1)
        assertAliases(client.admin().indices().prepareGetAliases(masterNodeTimeout).setIndices("test_1"), "test_1", "test_alias");

        // ok: user has manage_aliases on test_*, test_* aliases gets resolved to test_alias and empty indices gets resolved to _all
        // indices (thus test_1)
        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("test_*").setIndices("test_1"),
            "test_1",
            "test_alias"
        );
        ...
        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("_all").setIndices("_all"),
            "test_1",
            "test_alias"
        );

        // ok: user has manage_aliases on test_*, empty aliases gets resolved to test_alias and empty indices becomes test_1
        assertAliases(client.admin().indices().prepareGetAliases(masterNodeTimeout), "test_1", "test_alias");
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("_all", "alias_1")::get,
            GetAliasesAction.NAME,
            "create_test_aliases_test"
        );
        ...
        // fails: user doesn't have manage_aliases on alias_1
        assertThrowsAuthorizationException(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("alias_1")::get,
            GetAliasesAction.NAME,
            "create_test_aliases_test"
        );
    }
    ...
    public void testCreateIndexThenAliasesCreateAndAliasesPermission2() {
        ...
        // fails: user doesn't have manage_aliases aliases on test_1
        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_1", "alias_1")::get,
            TransportIndicesAliasesAction.NAME,
            "create_test_aliases_alias"
        );
        ...
        IndexNotFoundException indexNotFoundException = expectThrows(
            ...
            client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_*", "alias_1")::get
        );
    ...
    public void testDeleteAliasesCreateAndAliasesPermission2() {
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "test_alias")::get,
            TransportIndicesAliasesAction.NAME,
            "create_test_aliases_alias"
        );
        ...
        expectThrows(
            IndexNotFoundException.class,
            client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_*", "alias_1")::get
        );
    }
    ...
    public void testGetAliasesCreateAndAliasesPermission2() {
        ...
        assertThrowsAuthorizationException(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("test_alias").setIndices("test_1")::get,
            GetAliasesAction.NAME,
            "create_test_aliases_alias"
        );
        ...
        getAliasesResponse = client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("test_alias").get();
        assertEquals(0, getAliasesResponse.getAliases().size());

        {
            // fails: no existing aliases to replace wildcards
            assertThrowsAuthorizationException(
                client.admin().indices().prepareGetAliases(masterNodeTimeout).setIndices("test_1").setAliases("test_*")::get,
                GetAliasesAction.NAME,
                "create_test_aliases_alias"
            );
        }
        {
            ...
            assertThrowsAuthorizationException(
                client.admin().indices().prepareGetAliases(masterNodeTimeout).setIndices("test_1").setAliases("_all")::get,
                GetAliasesAction.NAME,
                "create_test_aliases_alias"
            );
        }
        {
            ...
            assertThrowsAuthorizationException(
                client.admin().indices().prepareGetAliases(masterNodeTimeout).setIndices("test_1")::get,
                GetAliasesAction.NAME,
                "create_test_aliases_alias"
            );
        }
        {
            // fails: no existing aliases to replace empty aliases
            GetAliasesResponse response = client.admin().indices().prepareGetAliases(masterNodeTimeout).get();
    ...
    public void testCreateIndexThenAliasesCreateAndAliasesPermission3() {
        ...
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_1", "alias_1"));

        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).addAlias("test_*", "alias_2"));
    }
    ...
    public void testDeleteAliasesCreateAndAliasesPermission3() {
        ...
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_1", "alias_1"));

        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).removeAlias("test_*", "_all"));
    ...
    public void testGetAliasesCreateAndAliasesPermission3() {
        ...
        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("alias_1").setIndices("test_1"),
            "test_1",
            "alias_1"
        );

        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("alias_1").setIndices("test_*"),
            "test_1",
            "alias_1"
        );
        ...
        assertAliases(client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("_all"), "test_1", "alias_1", "test_alias");

        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setIndices("test_1"),
            "test_1",
            "alias_1",
            "test_alias"
        );

        assertAliases(client.admin().indices().prepareGetAliases(masterNodeTimeout), "test_1", "alias_1", "test_alias");

        assertAliases(
            client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("alias_*").setIndices("test_*"),
            "test_1",
            "alias_1"
        );
    }
    ...
    public void testGetAliasesAliasesOnlyPermissionStrict() {
        ...
        IndexNotFoundException indexNotFoundException = expectThrows(
            ...
            client.admin()
                .indices()
                .prepareGetAliases(masterNodeTimeout, "alias_1")
                .addIndices("test_1")
        ...
        assertThrowsAuthorizationException(
            client.admin()
                .indices()
                .prepareGetAliases(masterNodeTimeout, "non_authorized")
                .addIndices("test_1")
                .setIndicesOptions(IndicesOptions.strictExpandOpen())::get,
            GetAliasesAction.NAME,
            "aliases_only"
    ...
    public void testRemoveIndex() {
        ...
        assertAliases(client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("*"), "test_1", "test_alias_1");

        // test that the remove index wildcard expacnds only to authorized indices
        assertAcked(client.admin().indices().prepareAliases(masterNodeTimeout).removeIndex("*").get());
        GetAliasesResponse getAliasesResponse = client.admin().indices().prepareGetAliases(masterNodeTimeout).setAliases("*").get();
        assertThat(getAliasesResponse.getAliases().size(), equalTo(0));
        assertAliases(
            indicesAdmin().prepareGetAliases(masterNodeTimeout).setAliases("*"),
            "bogus_index_1",
            "bogus_alias_1",
            "bogus_alias_2"
        );
    ...
    public void testAliasesForHiddenIndices() {
        ...
        final GetAliasesRequestBuilder req = aliasesClient.admin().indices().prepareGetAliases(masterNodeTimeout, visibleAlias);
        ...
        response = indicesAdmin().prepareGetAliases(masterNodeTimeout, "alias*").get();
        assertThat(response.getAliases().get(hiddenIndex), hasSize(1));
        assertThat(response.getAliases().get(hiddenIndex).get(0).alias(), Matchers.equalTo(visibleAlias));
        assertThat(response.getAliases().get(hiddenIndex).get(0).isHidden(), nullValue());
        ...
        response = aliasesClient.admin().indices().prepareGetAliases(masterNodeTimeout, hiddenAlias).get();
        assertThat(response.getAliases().get(hiddenIndex), hasSize(1));
        ...
        response = aliasesClient.admin().indices().prepareGetAliases(masterNodeTimeout, "alias*").get();
        assertThat(response.getAliases().get(hiddenIndex), hasSize(1));
        assertThat(response.getAliases().get(hiddenIndex).get(0).alias(), Matchers.equalTo(hiddenAlias));
        assertThat(response.getAliases().get(hiddenIndex).get(0).isHidden(), Matchers.equalTo(true));
        ...
        response = aliasesClient.admin()
            .indices()
            .prepareGetAliases(masterNodeTimeout, "alias*")
