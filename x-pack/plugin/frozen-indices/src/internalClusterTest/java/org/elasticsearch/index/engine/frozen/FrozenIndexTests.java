...
public class FrozenIndexTests extends ESSingleNodeTestCase {
    ...
    public void testIgnoreUnavailable() {
        ...
        assertEquals(
            ...
            clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index("idx-close").getState()
        );
    }
    ...
    public void testUnfreezeClosedIndex() {
        ...
        assertEquals(
            IndexMetadata.State.CLOSE,
            clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index("idx").getState()
        );
        expectThrows(
    ...
    public void testFreezeIndexIncreasesIndexSettingsVersion() {
        ...
        assertThat(
            clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index(index).getSettingsVersion(),
    ...
    public void testFreezeEmptyIndexWithTranslogOps() throws Exception {
        ...
        assertBusy(() -> {
            final Index index = clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index(indexName).getIndex();
    ...
    public void testRecoveryState() {
        ...
        final IndexMetadata indexMetadata = clusterAdmin().prepareState(masterNodeTimeout).get().getState().metadata().index(indexName);
