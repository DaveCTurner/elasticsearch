...
public class ArchiveLicenseIntegTests extends AbstractArchiveTestCase {
    ...
    public void testFeatureUsage() throws Exception {
        XPackUsageFeatureResponse usage = client().execute(XPackUsageFeatureAction.ARCHIVE, new XPackUsageRequest(masterNodeTimeout)).get();
        ...
        final RestoreSnapshotRequest req = new RestoreSnapshotRequest(masterNodeTimeout, repoName, snapshotName).indices(indexName)
        ...
        usage = client().execute(XPackUsageFeatureAction.ARCHIVE, new XPackUsageRequest(masterNodeTimeout)).get();
        assertThat(usage.getUsage(), instanceOf(ArchiveFeatureSetUsage.class));
    ...
    public void testFailRestoreOnInvalidLicense() throws Exception {
        assertAcked(client().execute(TransportDeleteLicenseAction.TYPE, new AcknowledgedRequest.Plain(masterNodeTimeout)).get());
