...
public class NodeShutdownShardsIT extends ESIntegTestCase {
    ...
    public void testNodeReplacementOnlyAllowsShardsFromReplacedNode() throws Exception {
        ...
        ClusterAllocationExplainResponse explainResponse = clusterAdmin().prepareAllocationExplain(masterNodeTimeout)
            .setIndex("other")
            .setShard(0)
            .setPrimary(false)
            .get();
    ...
    private void assertIndexPrimaryShardsAreAllocatedOnNode(String indexName, String nodeId) {
        var state = clusterAdmin().prepareState(masterNodeTimeout).clear().setRoutingTable(true).get().getState();
    ...
    private void assertIndexSetting(String index, String setting, String expectedValue) {
        var response = indicesAdmin().prepareGetSettings(masterNodeTimeout, index).get();
